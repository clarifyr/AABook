...
\begin{alltt}
\hlcom{#Code to be changed }
\hlcom{#Installing clarifyR package from GitHub}
\hlcom{#First get devtools package and load into memory}
\hlkwd{install.packages}\hlstd{(}\hlstr{"devtools"}\hlstd{);}\hlkwd{library}\hlstd{(devtools)}
\hlkwd{install_github}\hlstd{(}\hlstr{"clarifyr"}\hlstd{,} \hlkwc{username}\hlstd{=}\hlstr{"clarifyr"}\hlstd{)}
\hlkwd{library}\hlstd{(clarifyr)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{# This is a comment as it comes after #. }
\hlcom{#You should put enough of these}
\hlkwd{print}\hlstd{(}\hlstr{"This statement is incomplete"}
      \hlstd{)} \hlcom{#R managed to figure that the command was incomplete}
\end{alltt}
\begin{verbatim}
...
\hlkwd{curve}\hlstd{(x}\hlopt{^}\hlnum{2}\hlstd{,}\hlkwc{from} \hlstd{=} \hlnum{0}\hlstd{,}\hlkwc{to} \hlstd{=} \hlnum{3}\hlstd{,} \hlkwc{xlab} \hlstd{=} \hlstr{"Time"}\hlstd{,} \hlkwc{ylab}\hlstd{=}\hlstr{"Expertise"}\hlstd{,}
      \hlkwc{col} \hlstd{=} \hlstr{"red"}\hlstd{,} \hlkwc{lwd}\hlstd{=}\hlnum{2}\hlstd{,} \hlkwc{xaxt} \hlstd{=} \hlstr{'n'}\hlstd{,} \hlkwc{yaxt} \hlstd{=} \hlstr{'n'}\hlstd{)}
...
\end{alltt}
\begin{verbatim}
## [1] "C:/Home/R/win-library/3.3"         
## [2] "C:/Program Files/R/R-3.3.0/library"
\end{verbatim}
\begin{alltt}
\hlcom{# I have two folders in my library path currently}
\hlcom{# You can add your own library folder to the path}
\hlcom{#libPaths( c( .libPaths(), "C:/Whatever/Whatever...") )}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hlopt{?}\hlstr{"vector"} \hlcom{#or this simple command works too}
...
\hlkwd{install.packages}\hlstd{(}\hlstr{"installr"}\hlstd{);} \hlkwd{library}\hlstd{(installr)}
...
\hlnum{100} \hlkwb{->} \hlstd{revenue} \hlcom{#Reverse direction assignment}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{nchar}\hlstd{(day.name);} \hlkwd{nchar}\hlstd{(someday)}
\end{alltt}
\begin{verbatim}
## [1] 6
## [1] 0
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{plot}\hlstd{(}\hlkwc{x}\hlstd{=dep_delay[dep_delay} \hlopt{<} \hlnum{400}\hlstd{],} \hlkwc{y}\hlstd{=arr_delay[dep_delay} \hlopt{<} \hlnum{400}\hlstd{],}
     \hlkwc{xlab}\hlstd{=}\hlstr{"Departure Delay"}\hlstd{,} \hlkwc{ylab}\hlstd{=}\hlstr{"Arrival Delay"}\hlstd{)}
\end{alltt}
\end{kframe}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{trees}\hlopt{$}\hlstd{is.tall} \hlkwb{<-} \hlkwd{cut}\hlstd{(trees}\hlopt{$}\hlstd{Height,} \hlkwc{breaks} \hlstd{=} \hlkwd{c}\hlstd{(}\hlopt{-}\hlnum{Inf}\hlstd{,} \hlnum{80}\hlstd{,} \hlnum{Inf}\hlstd{),}
...
\hlkwd{step}\hlstd{(}\hlkwc{object} \hlstd{= emptymodel,} \hlkwc{direction} \hlstd{=} \hlstr{"forward"}\hlstd{,}
     \hlkwc{scope} \hlstd{=} \hlkwd{log}\hlstd{(Volume)} \hlopt{~} \hlkwd{log}\hlstd{(Girth)} \hlopt{+} \hlkwd{log}\hlstd{(Height))}
\end{alltt}
\begin{verbatim}
## Start:  AIC=-38.82
## log(Volume) ~ 1
...
\hlstd{model.A} \hlkwb{<-} \hlkwd{lm}\hlstd{(}\hlkwd{log}\hlstd{(Volume)} \hlopt{~} \hlkwd{log}\hlstd{(Girth)} \hlopt{+} \hlkwd{log}\hlstd{(Height),}
              \hlkwc{data} \hlstd{= trees)}
\hlstd{model.B} \hlkwb{<-} \hlkwd{lm}\hlstd{(}\hlkwd{log}\hlstd{(Volume)} \hlopt{~} \hlkwd{log}\hlstd{(Girth)} \hlopt{+} \hlkwd{log}\hlstd{(Height)}
              \hlopt{+} \hlstd{trees}\hlopt{$}\hlstd{random,} \hlkwc{data} \hlstd{= trees)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{AIC}\hlstd{(model.A);} \hlkwd{AIC}\hlstd{(model.B)}
\end{alltt}
\begin{verbatim}
## [1] -62.71125
## [1] -66.69933
...
\hlstd{odds} \hlkwb{=} \hlstd{probs} \hlopt{/} \hlstd{(}\hlnum{1} \hlopt{-} \hlstd{probs); logodds} \hlkwb{=} \hlkwd{log}\hlstd{(odds)}
...
\hlstd{logit.model} \hlkwb{<-} \hlkwd{glm}\hlstd{(admit} \hlopt{~} \hlstd{gre} \hlopt{+} \hlstd{gpa} \hlopt{+} \hlstd{rank,} \hlkwc{data} \hlstd{= admit.data,}
...
\hlkwd{wald.test}\hlstd{(}\hlkwc{b} \hlstd{=} \hlkwd{coef}\hlstd{(logit.model),} \hlkwc{Sigma} \hlstd{=} \hlkwd{vcov}\hlstd{(logit.model),}
          \hlkwc{Terms} \hlstd{=} \hlnum{4}\hlopt{:}\hlnum{6}\hlstd{)}
\end{alltt}
\begin{verbatim}
## Wald test:
## ----------
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{probit.model} \hlkwb{<-} \hlkwd{glm}\hlstd{(admit} \hlopt{~} \hlstd{gre} \hlopt{+} \hlstd{gpa} \hlopt{+} \hlstd{rank,} \hlkwc{data} \hlstd{= admit.data,}
...
\hlkwd{library}\hlstd{(OIsurv);} \hlkwd{library}\hlstd{(survival)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{mod.first} \hlkwb{<-} \hlkwd{coxph}\hlstd{(rossi.surv} \hlopt{~} \hlstd{fin} \hlopt{+} \hlstd{age} \hlopt{+} \hlstd{race} \hlopt{+} \hlstd{wexp}
                   \hlopt{+} \hlstd{mar} \hlopt{+} \hlstd{paro} \hlopt{+} \hlstd{prio} \hlopt{+} \hlstd{educ,} \hlkwc{data}\hlstd{=rossi)}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{plot}\hlstd{(}\hlkwd{survfit}\hlstd{(mod.first),} \hlkwc{ylim}\hlstd{=}\hlkwd{c}\hlstd{(}\hlnum{0.75}\hlstd{,} \hlnum{1}\hlstd{),}
     \hlkwc{xlab} \hlstd{=} \hlstr{"Weeks of Observation"}\hlstd{,}
     \hlkwc{ylab}\hlstd{=}\hlstr{"Proportion of Subject Not Rearrested"}\hlstd{)}
\end{alltt}
\end{kframe}
...
\hlstd{rossi.long} \hlkwb{<-} \hlkwd{reshape}\hlstd{(}\hlkwc{data} \hlstd{= rossi,} \hlkwc{varying} \hlstd{=} \hlkwd{paste0}\hlstd{(}\hlstr{"emp"}\hlstd{,} \hlnum{1}\hlopt{:}\hlnum{52}\hlstd{),}
                      \hlkwc{v.names} \hlstd{=} \hlstr{"employed"}\hlstd{,} \hlkwc{timevar} \hlstd{=} \hlstr{"time"}\hlstd{,}
                      \hlkwc{idvar} \hlstd{=} \hlstr{"id"}\hlstd{,} \hlkwc{direction} \hlstd{=} \hlstr{"long"}\hlstd{,} \hlkwc{sep} \hlstd{=} \hlstr{""}\hlstd{)}
...
\hlstd{rossi.long} \hlkwb{<-} \hlkwd{ddply}\hlstd{(}
  \hlkwc{.data} \hlstd{= rossi.long,} \hlkwc{.variables} \hlstd{=} \hlkwd{c}\hlstd{(}\hlstr{"id"}\hlstd{),} \hlkwc{.drop} \hlstd{=} \hlnum{TRUE}\hlstd{,} \hlkwc{.fun} \hlstd{=} \hlkwa{function}\hlstd{(}\hlkwc{DF}\hlstd{)}
  \hlstd{\{}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{mod.timed} \hlkwb{<-} \hlkwd{coxph}\hlstd{(}\hlkwd{Surv}\hlstd{(start, stop, event)} \hlopt{~} \hlstd{fin} \hlopt{+} \hlstd{age}
                   \hlopt{+} \hlstd{race} \hlopt{+} \hlstd{wexp} \hlopt{+} \hlstd{mar} \hlopt{+} \hlstd{paro} \hlopt{+} \hlstd{prio}
                   \hlopt{+} \hlstd{employed,} \hlkwc{data} \hlstd{= rossi.long)}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{mod.lagtimed} \hlkwb{<-} \hlkwd{coxph}\hlstd{(}\hlkwd{Surv}\hlstd{(start, stop, event)} \hlopt{~} \hlstd{fin} \hlopt{+} \hlstd{age}
                      \hlopt{+} \hlstd{race} \hlopt{+} \hlstd{wexp} \hlopt{+} \hlstd{mar} \hlopt{+} \hlstd{paro} \hlopt{+} \hlstd{prio} \hlopt{+}
                        \hlstd{employed.lag1,} \hlkwc{data}  \hlstd{= rossi.long)}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{mod.small} \hlkwb{<-} \hlkwd{coxph}\hlstd{(}\hlkwd{Surv}\hlstd{(week, arrest)} \hlopt{~} \hlstd{fin} \hlopt{+} \hlstd{age} \hlopt{+} \hlstd{prio,}
                   \hlkwc{data}\hlstd{=rossi)}
\hlstd{mod.small}
\end{alltt}
\begin{verbatim}
## Call:
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{par}\hlstd{(}\hlkwc{mfrow}\hlstd{=}\hlkwd{c}\hlstd{(}\hlnum{2}\hlstd{,}\hlnum{2}\hlstd{));} \hlkwd{plot}\hlstd{(}\hlkwd{cox.zph}\hlstd{(mod.small))}
\end{alltt}
\end{kframe}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{mod.int} \hlkwb{<-} \hlkwd{coxph}\hlstd{(}\hlkwd{Surv}\hlstd{(start, stop, event)} \hlopt{~} \hlstd{fin} \hlopt{+} \hlstd{age} \hlopt{+} \hlstd{prio}
                 \hlopt{+} \hlstd{age}\hlopt{:}\hlstd{stop,} \hlkwc{data}\hlstd{=rossi.long)}
\hlstd{mod.int}
\end{alltt}
\begin{verbatim}
## Call:
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{dfbeta} \hlkwb{<-} \hlkwd{residuals}\hlstd{(mod.small,} \hlkwc{type}\hlstd{=}\hlstr{"dfbeta"}\hlstd{);} \hlkwd{par}\hlstd{(}\hlkwc{mfrow}\hlstd{=}\hlkwd{c}\hlstd{(}\hlnum{2}\hlstd{,}\hlnum{2}\hlstd{))}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{#Number of International Passenger bookings (in '000s) for PanAm}
\hlkwd{data}\hlstd{(}\hlstr{"AirPassengers"}\hlstd{)}
\hlkwd{class}\hlstd{(AirPassengers)}
\end{alltt}
\begin{verbatim}
## [1] "ts"
\end{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{plot}\hlstd{(AirPassengers,} \hlkwc{ylab}\hlstd{=}\hlstr{"Passengers"}\hlstd{,} \hlkwc{xlab}\hlstd{=}\hlstr{"Years"}\hlstd{,}
     \hlkwc{main}\hlstd{=}\hlstr{"Passenger Bookings"}\hlstd{)}
\end{alltt}
\end{kframe}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{data}\hlstd{(}\hlstr{"lynx"}\hlstd{)} \hlcom{#Number of Lynx trappings}
\hlkwd{plot}\hlstd{(lynx,} \hlkwc{ylab}\hlstd{=}\hlstr{"Number of Lynx Trapped"}\hlstd{,} \hlkwc{main}\hlstd{=}\hlstr{"Lynx Trappings"}\hlstd{)}
\end{alltt}
\end{kframe}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{data}\hlstd{(}\hlstr{"lh"}\hlstd{)} \hlcom{#LH levels every 10 mins}
\hlkwd{plot}\hlstd{(lh,} \hlkwc{ylab}\hlstd{=}\hlstr{"LH Level"}\hlstd{,} \hlkwc{main}\hlstd{=}\hlstr{"Luteinizing Hormone"}\hlstd{)}
\end{alltt}
\end{kframe}
\includegraphics[width=\maxwidth]{figure/unnamed-chunk-104-1} 
\begin{kframe}\begin{alltt}
\hlkwd{plot}\hlstd{(lh[}\hlnum{1}\hlopt{:}\hlnum{47}\hlstd{], lh[}\hlnum{2}\hlopt{:}\hlnum{48}\hlstd{],} \hlkwc{pch}\hlstd{=}\hlnum{20}\hlstd{)}
\hlkwd{title}\hlstd{(}\hlstr{"Scatterplot with Lag 1"}\hlstd{)}
\end{alltt}
\end{kframe}
\includegraphics[width=\maxwidth]{figure/unnamed-chunk-104-2} 
\begin{kframe}\begin{alltt}
\hlkwd{cor}\hlstd{(lh[}\hlnum{1}\hlopt{:}\hlnum{47}\hlstd{], lh[}\hlnum{2}\hlopt{:}\hlnum{48}\hlstd{])}
\end{alltt}
\begin{verbatim}
## [1] 0.5807322
\end{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{#Default is yyyy-mm-dd}
\hlkwd{as.Date}\hlstd{(}\hlstr{"2016-02-29"}\hlstd{);} \hlkwd{as.Date}\hlstd{(}\hlstr{"2016/02/29"}\hlstd{)}
\end{alltt}
\begin{verbatim}
## [1] "2016-02-29"
## [1] "2016-02-29"
\end{verbatim}
\begin{alltt}
\hlkwd{as.Date}\hlstd{(}\hlstr{"29.02.16"}\hlstd{,} \hlkwc{format}\hlstd{=}\hlstr{"%d.%m.%y"}\hlstd{)}
\end{alltt}
\begin{verbatim}
## [1] "2016-02-29"
\end{verbatim}
\begin{alltt}
\hlkwd{as.Date}\hlstd{(}\hlstr{"29. Feb, 2016"}\hlstd{,} \hlkwc{format}\hlstd{=}\hlstr{"%d. %b, %Y"}\hlstd{)}
\end{alltt}
\begin{verbatim}
## [1] "2016-02-29"
\end{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{mydat} \hlkwb{<-} \hlkwd{as.Date}\hlstd{(}\hlstr{"2016-02-29"}\hlstd{)}
\hlkwd{weekdays}\hlstd{(mydat);} \hlkwd{months}\hlstd{(mydat);} \hlkwd{quarters}\hlstd{(mydat)}
\end{alltt}
\begin{verbatim}
## [1] "Monday"
## [1] "February"
...
\end{verbatim}
\begin{alltt}
\hlstd{currentdate} \hlkwb{<-} \hlkwd{Sys.Date}\hlstd{()} \hlcom{#Gives current date}
\hlstd{currentdate} \hlopt{-} \hlstd{mydat}
\end{alltt}
\begin{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{data}\hlstd{(}\hlstr{"co2"}\hlstd{);} \hlkwd{plot}\hlstd{(co2,} \hlkwc{main}\hlstd{=}\hlstr{"CO2 Concentrations"}\hlstd{)}
\end{alltt}
\end{kframe}
\includegraphics[width=\maxwidth]{figure/unnamed-chunk-107-1} 
\begin{kframe}\begin{alltt}
\hlstd{sd.co2} \hlkwb{<-} \hlkwd{diff}\hlstd{(co2,} \hlkwc{lag} \hlstd{=} \hlnum{12}\hlstd{)}
\hlkwd{plot}\hlstd{(sd.co2,} \hlkwc{main}\hlstd{=}\hlstr{"Differenced with Lag 12 months"}\hlstd{)}
\end{alltt}
\end{kframe}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{co2.dec} \hlkwb{<-} \hlkwd{decompose}\hlstd{(co2)}
\hlkwd{plot}\hlstd{(co2.dec)}
\end{alltt}
\end{kframe}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{co2.stl} \hlkwb{<-} \hlkwd{stl}\hlstd{(co2,} \hlkwc{s.window} \hlstd{=} \hlstr{"periodic"}\hlstd{)}
\hlkwd{plot}\hlstd{(co2.stl,} \hlkwc{main}\hlstd{=}\hlstr{"STL Decomposition"}\hlstd{)}
\end{alltt}
\end{kframe}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{wave} \hlkwb{<-} \hlkwd{ts}\hlstd{(}\hlkwd{read.table}\hlstd{(}\hlstr{"http://staff.elena.aut.ac.nz/Paul-Cowpertwait/ts/wave.dat"}\hlstd{,}
                      \hlkwc{header}\hlstd{=}\hlnum{TRUE}\hlstd{))}
\end{alltt}


{\ttfamily\noindent\color{warningcolor}{\#\# Warning in file(file, "{}rt"{}): InternetOpenUrl failed: 'The server name or address could not be resolved'}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in file(file, "{}rt"{}): cannot open the connection}}\begin{alltt}
\hlkwd{plot}\hlstd{(}\hlkwd{window}\hlstd{(wave,} \hlnum{1}\hlstd{,} \hlnum{60}\hlstd{),} \hlkwc{ylab}\hlstd{=}\hlstr{"Height"}\hlstd{,} \hlkwc{main}\hlstd{=}\hlstr{"Wave Tank Data"}\hlstd{)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in window(wave, 1, 60): object 'wave' not found}}\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{pacf}\hlstd{(wave,} \hlkwc{main}\hlstd{=}\hlstr{"Partial Autocorrelation of Wave Tank Data"}\hlstd{)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in pacf(wave, main = "{}Partial Autocorrelation of Wave Tank Data"{}): object 'wave' not found}}\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{library}\hlstd{(nutshell);} \hlkwd{data}\hlstd{(turkey.price.ts)} \hlcom{#Get the data}
\hlstd{armodel} \hlkwb{<-} \hlkwd{ar}\hlstd{(turkey.price.ts); armodel} \hlcom{#Fit an AR model}
\end{alltt}
\begin{verbatim}
## 
## Call:
...
\end{verbatim}
\begin{alltt}
\hlkwd{predict}\hlstd{(armodel,} \hlkwc{n.ahead}\hlstd{=}\hlnum{12}\hlstd{)} \hlcom{#Predict future values, plot below}
\end{alltt}
\begin{verbatim}
## $pred
...
\end{verbatim}
\begin{alltt}
\hlkwd{ts.plot}\hlstd{(turkey.price.ts,} \hlkwd{predict}\hlstd{(armodel,} \hlkwc{n.ahead} \hlstd{=} \hlnum{24}\hlstd{)}\hlopt{$}\hlstd{pred,} \hlkwc{lty}\hlstd{=}\hlkwd{c}\hlstd{(}\hlnum{1}\hlopt{:}\hlnum{2}\hlstd{))}
\end{alltt}
\end{kframe}
...
\hlstd{arima.model} \hlkwb{<-} \hlkwd{auto.arima}\hlstd{(turkey.price.ts); arima.model}
\end{alltt}
\begin{verbatim}
...
\end{verbatim}
\begin{alltt}
\hlkwd{ts.plot}\hlstd{(turkey.price.ts,} \hlkwd{predict}\hlstd{(arima.model,}
                                 \hlkwc{n.ahead} \hlstd{=} \hlnum{24}\hlstd{)}\hlopt{$}\hlstd{pred,} \hlkwc{lty}\hlstd{=}\hlkwd{c}\hlstd{(}\hlnum{1}\hlopt{:}\hlnum{2}\hlstd{))}
\end{alltt}
\end{kframe}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{skirts} \hlkwb{<-} \hlkwd{scan}\hlstd{(}\hlstr{"http://robjhyndman.com/tsdldata/roberts/skirts.dat"}\hlstd{,}\hlkwc{skip}\hlstd{=}\hlnum{5}\hlstd{)}
\hlstd{skirtsseries} \hlkwb{<-} \hlkwd{ts}\hlstd{(skirts,}\hlkwc{start}\hlstd{=}\hlkwd{c}\hlstd{(}\hlnum{1866}\hlstd{))} \hlcom{#Create a ts object}
\hlkwd{plot.ts}\hlstd{(skirtsseries)} \hlcom{#plot the ts object}
\end{alltt}
\end{kframe}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{skirtsseriesdiff1} \hlkwb{<-} \hlkwd{diff}\hlstd{(skirtsseries,} \hlkwc{differences}\hlstd{=}\hlnum{1}\hlstd{)}
\hlkwd{plot.ts}\hlstd{(skirtsseriesdiff1)}
\end{alltt}
\end{kframe}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{skirtsseriesdiff2} \hlkwb{<-} \hlkwd{diff}\hlstd{(skirtsseries,} \hlkwc{differences}\hlstd{=}\hlnum{2}\hlstd{)}
\hlkwd{plot.ts}\hlstd{(skirtsseriesdiff2)}
\end{alltt}
\end{kframe}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{kings} \hlkwb{<-} \hlkwd{scan}\hlstd{(}\hlstr{"http://robjhyndman.com/tsdldata/misc/kings.dat"}\hlstd{,}\hlkwc{skip}\hlstd{=}\hlnum{3}\hlstd{)}
\hlstd{kingstimeseries} \hlkwb{<-} \hlkwd{ts}\hlstd{(kings)} \hlcom{#Convert to ts object}
\hlkwd{plot.ts}\hlstd{(kingstimeseries)} \hlcom{#plotting the time series}
\end{alltt}
\end{kframe}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{kingtimeseriesdiff1} \hlkwb{<-} \hlkwd{diff}\hlstd{(kingstimeseries,} \hlkwc{differences}\hlstd{=}\hlnum{1}\hlstd{)}
\hlkwd{plot.ts}\hlstd{(kingtimeseriesdiff1)}
\end{alltt}
\end{kframe}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{acf}\hlstd{(kingtimeseriesdiff1)} \hlcom{#Get the correlogram chart}
\end{alltt}
\end{kframe}
\includegraphics[width=\maxwidth]{figure/unnamed-chunk-121-1} 
\begin{kframe}\begin{alltt}
\hlkwd{acf}\hlstd{(kingtimeseriesdiff1,} \hlkwc{plot}\hlstd{=}\hlnum{FALSE}\hlstd{)} \hlcom{#Get the data, not the chart}
\end{alltt}
\begin{verbatim}
## 
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{volcanodust} \hlkwb{<-} \hlkwd{scan}\hlstd{(}\hlstr{"http://robjhyndman.com/tsdldata/annual/dvi.dat"}\hlstd{,} \hlkwc{skip}\hlstd{=}\hlnum{1}\hlstd{)}
\hlstd{volcanodustseries} \hlkwb{<-} \hlkwd{ts}\hlstd{(volcanodust,}\hlkwc{start}\hlstd{=}\hlkwd{c}\hlstd{(}\hlnum{1500}\hlstd{))}
\hlkwd{plot.ts}\hlstd{(volcanodustseries)}
\end{alltt}
\end{kframe}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{acf}\hlstd{(volcanodustseries,} \hlkwc{lag.max}\hlstd{=}\hlnum{20}\hlstd{)}
\end{alltt}
\end{kframe}
\includegraphics[width=\maxwidth]{figure/unnamed-chunk-123-1} 
\begin{kframe}\begin{alltt}
\hlkwd{acf}\hlstd{(volcanodustseries,} \hlkwc{lag.max}\hlstd{=}\hlnum{20}\hlstd{,} \hlkwc{plot}\hlstd{=}\hlnum{FALSE}\hlstd{)}
\end{alltt}
\begin{verbatim}
## 
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{pacf}\hlstd{(kingtimeseriesdiff1)} \hlcom{#Get the correlogram chart}
\end{alltt}
\end{kframe}
\includegraphics[width=\maxwidth]{figure/unnamed-chunk-124-1} 
\begin{kframe}\begin{alltt}
\hlkwd{pacf}\hlstd{(kingtimeseriesdiff1,} \hlkwc{plot}\hlstd{=}\hlnum{FALSE}\hlstd{)} \hlcom{#Get the data, not the chart}
\end{alltt}
\begin{verbatim}
## 
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{pacf}\hlstd{(volcanodustseries,} \hlkwc{lag.max}\hlstd{=}\hlnum{20}\hlstd{)}
\end{alltt}
\end{kframe}
\includegraphics[width=\maxwidth]{figure/unnamed-chunk-125-1} 
\begin{kframe}\begin{alltt}
\hlkwd{pacf}\hlstd{(volcanodustseries,} \hlkwc{lag.max}\hlstd{=}\hlnum{20}\hlstd{,} \hlkwc{plot}\hlstd{=}\hlnum{FALSE}\hlstd{)}
\end{alltt}
\begin{verbatim}
## 
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{# fit an ARIMA(0,1,1) model}
\hlstd{kingstimeseriesarima} \hlkwb{<-} \hlkwd{arima}\hlstd{(kingstimeseries,} \hlkwc{order}\hlstd{=}\hlkwd{c}\hlstd{(}\hlnum{0}\hlstd{,}\hlnum{1}\hlstd{,}\hlnum{1}\hlstd{))}
\hlstd{kingstimeseriesarima}
\end{alltt}
\begin{verbatim}
## 
## Call:
...
\begin{alltt}
\hlkwd{library}\hlstd{(forecast)}
\hlstd{kingstimeseriesforecasts} \hlkwb{<-} \hlkwd{forecast.Arima}\hlstd{(kingstimeseriesarima,} \hlkwc{h}\hlstd{=}\hlnum{5}\hlstd{)}
\hlstd{kingstimeseriesforecasts}
\end{alltt}
\begin{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{plot.forecast}\hlstd{(kingstimeseriesforecasts)}
\end{alltt}
\end{kframe}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{acf}\hlstd{(kingstimeseriesforecasts}\hlopt{$}\hlstd{residuals,} \hlkwc{lag.max}\hlstd{=}\hlnum{20}\hlstd{)}
\end{alltt}
\end{kframe}
\includegraphics[width=\maxwidth]{figure/unnamed-chunk-129-1} 
\begin{kframe}\begin{alltt}
\hlkwd{Box.test}\hlstd{(kingstimeseriesforecasts}\hlopt{$}\hlstd{residuals,} \hlkwc{lag}\hlstd{=}\hlnum{20}\hlstd{,} \hlkwc{type}\hlstd{=}\hlstr{"Ljung-Box"}\hlstd{)}
\end{alltt}
\begin{verbatim}
## 
## 	Box-Ljung test
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{# make time plot of forecast errors}
\hlkwd{plot.ts}\hlstd{(kingstimeseriesforecasts}\hlopt{$}\hlstd{residuals)}
\end{alltt}
\end{kframe}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{automodel} \hlkwb{<-} \hlkwd{auto.arima}\hlstd{(kingstimeseries)}
\hlstd{automodel}
\end{alltt}
\begin{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{automodel.with.bic} \hlkwb{<-} \hlkwd{auto.arima}\hlstd{(kingstimeseries,} \hlkwc{ic}\hlstd{=}\hlstr{"bic"}\hlstd{)}
\hlstd{automodel.with.bic}
\end{alltt}
\begin{verbatim}
## Series: kingstimeseries 
## ARIMA(0,1,1)                    
...
\hlcom{#str(wbcd) # Lets check what we have in this dataset}
\hlcom{# Str command commented for brevity in output}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\end{verbatim}
\begin{alltt}
\hlstd{wbcd}\hlopt{$}\hlstd{diagnosis} \hlkwb{<-} \hlkwd{factor}\hlstd{(wbcd}\hlopt{$}\hlstd{diagnosis,} \hlkwc{levels} \hlstd{=} \hlkwd{c}\hlstd{(}\hlstr{"B"}\hlstd{,} \hlstr{"M"}\hlstd{),}
                         \hlkwc{labels} \hlstd{=} \hlkwd{c}\hlstd{(}\hlstr{"Benign"}\hlstd{,} \hlstr{"Malignant"}\hlstd{))}
...
{\ttfamily\noindent\color{warningcolor}{\#\# Warning: package 'class' was built under R version 3.3.1}}\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{wbcd_test_pred} \hlkwb{<-} \hlkwd{knn}\hlstd{(}\hlkwc{train} \hlstd{= wbcd_train,} \hlkwc{test} \hlstd{= wbcd_test,}
                      \hlkwc{cl} \hlstd{= wbcd_train_labels,} \hlkwc{k}\hlstd{=}\hlnum{21}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\color{warningcolor}{\#\# Warning: package 'gmodels' was built under R version 3.3.1}}\begin{alltt}
\hlkwd{CrossTable}\hlstd{(}\hlkwc{x} \hlstd{= wbcd_test_labels,} \hlkwc{y} \hlstd{= wbcd_test_pred,}
           \hlkwc{prop.chisq}\hlstd{=}\hlnum{FALSE}\hlstd{)}
\end{alltt}
\begin{verbatim}
## 
##  
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{wbcd_train} \hlkwb{<-} \hlstd{wbcd_z[}\hlnum{1}\hlopt{:}\hlnum{469}\hlstd{, ]; wbcd_test} \hlkwb{<-} \hlstd{wbcd_z[}\hlnum{470}\hlopt{:}\hlnum{569}\hlstd{, ]}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{nrow}\hlstd{(nycdata);} \hlkwd{ncol}\hlstd{(nycdata)}
\end{alltt}
\begin{verbatim}
## [1] 27004
## [1] 17
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{median}\hlstd{(nycdata}\hlopt{$}\hlstd{arr_delay,} \hlkwc{na.rm}\hlstd{=}\hlnum{TRUE}\hlstd{);} \hlkwd{IQR}\hlstd{(nycdata}\hlopt{$}\hlstd{arr_delay,} \hlkwc{na.rm}\hlstd{=}\hlnum{TRUE}\hlstd{)}
\end{alltt}
\begin{verbatim}
## [1] -3
## [1] 28
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{delaybycarrier} \hlkwb{<-} \hlkwd{tapply}\hlstd{(nycdata}\hlopt{$}\hlstd{arr_delay, nycdata}\hlopt{$}\hlstd{carrier,}
...
\hlkwd{library}\hlstd{(plyr);} \hlkwd{library}\hlstd{(dplyr)}
\end{alltt}
\end{kframe}
\end{knitrout}
